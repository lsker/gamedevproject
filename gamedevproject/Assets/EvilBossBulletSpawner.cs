...
public class EvilBossBulletSpawner : MonoBehaviour
{
...
/*                float xval = initDistance * Mathf.Cos(currentAngle);
    ...
    */
        ...
        IEnumerator AttackScript(){
            ...
            while(isntDone){
                startText.SetActive(true); //Welcome to the new job...
                yield return new WaitForSeconds(3);                
                startText.SetActive(false);
                //secondText.SetActive(true); 
                while(!hasResponded){
                    //attackButton.SetActive(true);
                    secondText.SetActive(true); //Are you ready to destroy the indie wiki association?
                    goodResponseButton.SetActive(true);
                    badResponseButton.SetActive(true);
                    yield return new WaitForSeconds(3);
                }
                if(goodAttackTaken==false){
                    failedInterviewText.SetActive(true); //Seems like you won't fit into our company's family culture!
                ...
                if(goodAttackTaken==true){
                    interviewPassedText.SetActive(true);
                    yield return new WaitForSeconds(3);
                    interviewPassedText.SetActive(false);
...
//                    attackIndieWikiBuddyTextPart2.SetActive(true);
    ...
    void Update(){
        ...
        if(goToVictoryScene){
            SceneManager.LoadScene("SecondStreetScene");
        }
    }
